...
class VideoSearchPipeline:
    ...
    def __init__(
        ...
        frames_dir: Union[str, Path] = "./data/processed/extracted_frames",
    ) -> None:
    ...
    def search(
        ...
        # Check audio score variance across candidate hits (only mask if audio scores are low noise)
        audio_scores_list = list(audio_scores.values())
        low_audio_variance = False
        if max_raw_a < 0.30 and len(audio_scores_list) > 1:
        ...
        # Stage 3: Temporal Window Merging & De-duplication across contiguous hits
        top_results = merge_temporal_windows(top_results, max_delta_sec=5.0)[:top_k]

        # Stage 4: Attach confidence_band and match_rationale to each result
    ...
    def _get_confidence_band(result: Dict[str, Any]) -> str:
        """Map a result's fused score to a calibrated qualitative confidence band (HIGH / MEDIUM / LOW)."""
        score = result.get("score", 0.0)
        rrf = result.get("rrf_score", 0.0)
        v = result.get("visual_score", 0.0)
        a = result.get("audio_score", 0.0)
        return get_calibrated_confidence_band(score, rrf, v, a)
