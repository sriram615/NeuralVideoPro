...
def _build_xai(
    ...
    """Build XAI attribution breakdown for a single search result."""
    v_pct = round(visual_score * 100, 1)
    a_pct = round(audio_score * 100, 1)
    f_pct = round(fused_score * 100, 1)

    # Confidence tier
    if f_pct >= 70.0 or (rrf_score and rrf_score >= 0.02):
        tier = "high"
    elif f_pct >= 35.0 or (rrf_score and rrf_score >= 0.01):
        tier = "moderate"
    else:
        tier = "low"
    ...
    if effective_alpha >= 0.70:
        driver = "Visual Scene Match (CLIP)"
        emoji = "👁️"
    elif effective_alpha <= 0.30:
        driver = "Spoken Dialogue Match (Whisper)"
        emoji = "🎧"
    else:
        driver = "Balanced Hybrid Match (RRF)"
        emoji = "⚖️"

    formula = (
        f"RRF = ({effective_alpha:.2f} / (60 + r_v)) + "
        f"({1.0 - effective_alpha:.2f} / (60 + r_a)) = {rrf_score if rrf_score is not None else fused_score:.4f}"
    )
...
async def lifespan(app: FastAPI):
    ...
    db_path = os.getenv("QDRANT_DB_PATH", "./data/qdrant_db")
...
_static_raw = Path("./data/raw_videos")
...
_static_frames = Path("./data/extracted_frames")
